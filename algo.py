...
def place_stone(event):
    ...
    if touched_point.status == 'empty':
        ...
        b1 = touched_point.death_decision(test = True) # Chek if the touched point makes the cnected component die 
        ...
        if b1:
            ...
            for n in touched_point.neighbors:
                touched_point.status = colour
                b2 = b2 and not(n.death_decision(test = True))
                touched_point.status = 'empty'
            ...
            if b2:
                ...
                count = False
                stone = canvas.create_oval(x-1e-16, y-1e-16, x+1e-16, y+1e-16, fill=colour)

...
class GridDemo(ShowBase):
    ...
    def calculate_territory(self, color):
        """Calculate territory as empty spaces surrounded by stones of the given color."""
        territory = 0
        visited = set()
            ...
            for y in range(self.size):
                for z in range(self.size):
                    ...
                    if pos not in self.balls and pos not in visited:
                        # Simple flood-fill to check if surrounded by one color
        ...
        for x in range(self.size):
            for y in range(self.size):
                for z in range(self.size):
                    ...
                    #collision_node.show() #show the collison_nodes, bad performance althougt I like how it looks 
                    if x + 1 < self.size:
    ...
    def gui_extra(self):
        if self.extra_show == 0:
            ...
            gui_definitions = [
                #Grid Color
                ("color_label", OnscreenText, {"text": "Grid color, r,g,b", "pos": (0.5, -0.20), "scale": 0.07, "align": TextNode.ALeft, "font": self.custom_font}),
                ("color_entry", DirectEntry, {"text": "", "scale": 0.05, "command": self.custom_grid_color, "initialText": "", "pos": (0.2, 0, -0.2), "frameColor": (0, 0, 0, 0.5), "text_fg": (1, 1, 1, 1), "width": 6}),
                ...
                #Background color
                ("background_label", OnscreenText, {"text": "Background color, r,g,b", "pos": (0.5, -0.30), "scale": 0.07, "align": TextNode.ALeft, "font": self.custom_font}),
                ("background_entry", DirectEntry, {"text": "", "scale": 0.05, "command": self.custom_background_color, "initialText": "", "pos": (0.2, 0, -0.3), "frameColor": (0, 0, 0, 0.5), "text_fg": (1, 1, 1, 1), "width": 6}),
                ...
                #Komi
                ("komi_label", OnscreenText, {"text": "Komi", "pos": (0.5, -0.4), "scale": 0.07, "align": TextNode.ALeft, "font": self.custom_font}),
                ("komi_entry", DirectEntry, {"text": "", "scale": 0.05, "command": self.set_komi, "initialText": "", "pos": (0.2, 0, -0.4), "frameColor": (0, 0, 0, 0.5), "text_fg": (1, 1, 1, 1), "width": 3}),
                #Timer stuff
                ("timer_label", OnscreenText, {"text": "Time (s)", "pos": (0.5, -0.60), "scale": 0.07, "align": TextNode.ALeft, "font": self.custom_font}),
                ("timer_mode_menu", DirectOptionMenu, {"scale": 0.05, "items": ["No timer", "Absolute", "Fischer", "Byo-yomi"], "command": self.set_timer_mode, "initialitem": 0, "pos": (0.2, 0, -0.5), "frameColor": (0, 0, 0, 0.5), "text_fg": (1, 1, 1, 1)}),
                ("main_time_entry", DirectEntry, {"text": "", "scale": 0.05, "command": self.set_main_time, "initialText": "300", "pos": (0.2, 0, -0.6), "frameColor": (0, 0, 0, 0.5), "text_fg": (1, 1, 1, 1), "width": 4}),
                ("main_time_label", OnscreenText, {"text": "Time Method", "pos": (0.5, -0.50), "scale": 0.07, "align": TextNode.ALeft, "font": self.custom_font}),
                ("increment_entry", DirectEntry, {"text": "", "scale": 0.05, "command": self.set_increment, "initialText": "0", "pos": (0.2, 0, -0.7), "frameColor": (0, 0, 0, 0.5), "text_fg": (1, 1, 1, 1), "width": 4}),
                ("increment_label", OnscreenText, {"text": "Increment (s)", "pos": (0.5, -0.70), "scale": 0.07, "align": TextNode.ALeft, "font": self.custom_font}),
                ("byo_time_entry", DirectEntry, {"text": "", "scale": 0.05, "command": self.set_byo_time, "initialText": "30", "pos": (0.2, 0, -0.8), "frameColor": (0, 0, 0, 0.5), "text_fg": (1, 1, 1, 1), "width": 4}),
                ("byo_time_label", OnscreenText, {"text": "Byo-yomi Time (s)", "pos": (0.5, -0.80), "scale": 0.07, "align": TextNode.ALeft, "font": self.custom_font}),
                ("byo_periods_entry", DirectEntry, {"text": "", "scale": 0.05, "command": self.set_byo_periods, "initialText": "5", "pos": (0.2, 0, -0.9), "frameColor": (0, 0, 0, 0.5), "text_fg": (1, 1, 1, 1), "width": 4}),
                ("byo_periods_label", OnscreenText, {"text": "Byo-yomi Periods", "pos": (0.5, -0.9), "scale": 0.07, "align": TextNode.ALeft, "font": self.custom_font}),
            ]
    ...
    def setup_gui(self):#GUI
        ...
        self.turn_text = OnscreenText(text=f"Turn: {self.turn}",pos=(-1.9, 0.7), scale=0.07, align=TextNode.ALeft,mayChange=True)
        self.text_points = OnscreenText(text=f"Black points: {self.black_points}\nWhite points: {self.white_points}",pos=(1.9, 0.90), scale=0.07, align=TextNode.ARight,font= self.custom_font,mayChange=True)
        self.text_captures = OnscreenText(text=f"Black captures: {self.black_captures}\nWhite captures: {self.white_captures}",pos=(1.9, 0.7), scale=0.07, align=TextNode.ARight,font= self.custom_font,mayChange=True)
        self.help = OnscreenText(text="Press X for help",pos=(-1.9, 0.80), scale=0.07, align=TextNode.ALeft,font= self.custom_font)
        self.extra = OnscreenText(text="Press C for timer and extra settings",pos=(-1.9, 0.90), scale=0.07, align=TextNode.ALeft,font= self.custom_font)
        self.coord_label = OnscreenText(  text="Blue ball = (0,0,0)", pos=(0, 0.9), scale=0.06, align=TextNode.ACenter,font= self.custom_font)

    ##### Instructions for coordinate input #####
        self.coord_label = OnscreenText(
            text="Enter x,y,z", pos=(-1.9,-0.90), scale=0.07, align=TextNode.ALeft,font= self.custom_font
        )
        ...
        self.submit_button_move = DirectButton( # Create button to submit coordinates
            text="Submit move", scale=0.05, command=self.process_coordinates,
            pos=(-1.15,0, -0.9), frameColor=(0, 1, 0, 0.5), text_fg=(1, 1, 1, 1)
        )
    ##### Instructions for coordinate input #####    
    ...
    def process_coordinates(self, input_text=None):
        # If input_text is not a string (e.g., an event object), ignore it and use entry text
    ...
    def get_group(self, pos, color, visited=None, balls=None):
        ...
        while to_check:
            ...
            visited.add(current)
            group.add(current)
            print(f"Adding to group at {current}, color: {color}")  # Debug
            to_check.extend(self.get_adjacent_positions(current))
        print(f"Group identified: {group}")  # Debug
        return group
    ...
    def spawn_model(self, pos):
        ...
        self.turn_text.setText(f"Turn: {self.turn}")
        if self.timer_mode != "No Timer": #When a time that's not No time is being used
                self.pause_timer()  # Pause current player's timer
                self.start_timer()  # Start next player's timer

        elif(self.timer_mode == "Byo-yomi"):# If byo_yomi is being used
            # Reset byo-yomi time for the player who just moved
    ...
    def check_all_groups_for_captures(self):
        ...
        for pos in list(self.balls.keys()):
            if pos not in visited:
                ...
                print(f"Checking group {group} at {pos}, color {color}, has liberty: {has_lib}")  # Debug
                if group and not has_lib:
                    captured_groups.append((group, color))
                visited.update(group)

        for group, color in captured_groups:
            if color == 0:  # White captures Black group
                self.white_captures += len(group)
                print(f"White captured {len(group)} black stones, total: {self.white_captures}")  # Debug
            else:  # Black captures White group
                self.black_captures += len(group)
                print(f"Black captured {len(group)} white stones, total: {self.black_captures}")  # Debug
            self.remove_group(group)
            self.text_captures.setText(f"Black captures: {self.black_captures}\nWhite captures: {self.white_captures}")

        # Final update to ensure the display reflects the latest state
        self.text_captures.setText(f"Black captures: {self.black_captures}\nWhite captures: {self.white_captures}")
        print(f"B:{self.black_captures}\nW:{self.white_captures}")
    ...
    def is_legal_move(self, pos, color):
        ...
        # Apply simulated captures to actual state before ko check
        ...
        for pos in group:
            ...
            for adj in adj_positions:
                if adj not in balls and adj not in group:
                    ...
                    print(f"Liberty found at {adj} for group at {pos}")  # Debug
        has_lib = bool(liberties)
        print(f"Group {group} has liberty: {has_lib}, liberties: {liberties}")  # Debug
        return has_lib
    ...
    def plane_up(self):
        if (self.last_plane_action==0): # When the player for some reason goes e,w,q,w,e,w,q like showing one plane then everything and such but it does it by alternating the keys
            ...
            self.show_one_floor(0, current_z=self.current_plane)# in this case the upper one or the lower one in the other case.
    ...
    def plane_down(self):
        ...
        self.last_plane_action=0
    def show_one_floor(self, Up_or_down, current_z=None):
        if Up_or_down not in (0, 1):
